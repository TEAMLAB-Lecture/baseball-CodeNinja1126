...
def is_between_100_and_999(user_input_number):
    ...
    temp_number = int(user_input_number)
    result = True if 100 <= temp_number and temp_number < 1000 \
            else False
...
def is_duplicated_number(three_digit):
    ...
    for i in range(2):
        if three_digit.count(three_digit[i]) > 1:
            result = True
            break
    else:
        result = False
...
def get_not_duplicated_three_digit_number():
    ...
    temp_num = str(get_random_number())
    while is_duplicated_number(temp_num):
        temp_num = str(get_random_number())
...
def is_yes(one_more_input):
    ...
    temp_str = one_more_input.lower()

    if temp_str == 'y' or temp_str == 'yes':
        result = True
    else:
        result = False
